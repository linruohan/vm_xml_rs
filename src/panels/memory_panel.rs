...
impl MemoryPanel {
    pub fn show(ui: &mut egui::Ui, config: &mut VMConfig, colors: &ThemeColors) {
        ...
        card_group(ui, "最大内存 (热插拔)", None, colors, |ui| {
        ...
        card_group(ui, "当前内存 (动态气球)", None, colors, |ui| {
            ...
            if let Some(ref mut current_mem) = config.general.current_memory {
                ...
                grid(ui, "current_memory_grid", 2, |ui| {
                    ui.label("当前内存:");
                    ui.add(egui::Slider::new(&mut current_mem.value, 1..=128).text("GiB"));
                    ui.end_row();

                    ui.label("单位:");
                    let unit = current_mem.unit.get_or_insert_with(|| "GiB".to_string());
        ...
        card_group(ui, "内存后端配置", None, colors, |ui| {
            ...
            if let Some(ref mut backing) = config.memory_backing {
                ...
                egui::ComboBox::from_id_source("memory_access_mode")
                    ...
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut access_mode.mode, "shared".to_string(), "shared");
                        ui.selectable_value(
                ...
                let alloc_mode = backing.allocation.get_or_insert_with(|| {
                    crate::model::MemoryAllocation { mode: "ondemand".to_string(), threads: None }
                });
