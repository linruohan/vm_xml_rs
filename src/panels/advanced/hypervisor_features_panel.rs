    ...
    pub fn show(ui: &mut egui::Ui, config: &mut VMConfig, colors: &ThemeColors) {
        ...
        ui.with_layout(Layout::left_to_right(Align::TOP).with_main_wrap(true), |ui| {
                    ...
                    card_group(ui, "基础特性", Some("🔧"), colors, |ui| {
                        ...
                        if let Some(ref mut features) = config.hypervisor_features {
                            ...
                            grid(ui, "basic_features_grid", 2, |ui| {
                                ...
                                ui.end_row();
                                feature_toggle(ui, "HAP", &mut features.hap);
                                ui.end_row();
                                ...
                                ui.end_row();
                                feature_toggle(ui, "PMU", &mut features.pmu);
                                ui.end_row();
                                feature_toggle(ui, "VMport", &mut features.vmport);
                                ui.end_row();
                                feature_toggle(ui, "VMcoreinfo", &mut features.vmcoreinfo);
                                ui.end_row();
                                feature_toggle(ui, "HTM", &mut features.htm);
                                ui.end_row();
                                feature_toggle(ui, "Nested-HV", &mut features.nested_hv);
                                ui.end_row();
            ...
            ui.allocate_ui_with_layout(
                ...
                Layout::top_down(Align::LEFT),
                |ui| {
                    card_group(ui, "Hyper-V 特性", Some("🔷"), colors, |ui| {
                        if let Some(ref mut features) = config.hypervisor_features {
                            let mut has_hyperv = features.hyperv.is_some();
                            if checkbox(ui, &mut has_hyperv, "启用 Hyper-V") {
                                if has_hyperv {
                            ...
                            if let Some(ref mut hyperv) = features.hyperv {
                                ...
                                grid(ui, "hyperv_grid", 2, |ui| {
                                    ui.label("模式:");
                                    let mode =
                                        hyperv.mode.get_or_insert_with(|| "custom".to_string());
                                ...
                                ui.collapsing("特性开关", |ui| {
                                    grid(ui, "hyperv_features_grid", 2, |ui| {
                                        ...
                                        ui.end_row();
                                        feature_state_toggle(
                                            ui,
                                            "Frequencies",
                                            &mut hyperv.frequencies,
                                        );
                                        ui.end_row();
                                        feature_state_toggle(
                                            ui,
                                            "Reenlightenment",
                                            &mut hyperv.reenlightenment,
                                        );
                                        ui.end_row();
                                        ...
                                        feature_state_toggle(
                                            ui,
                                            "EMSR Bitmap",
                                            &mut hyperv.emsr_bitmap,
                                        );
                                        ui.end_row();
                                        feature_state_toggle(
                                            ui,
                                            "XMM Input",
                                            &mut hyperv.xmm_input,
                                        );
                                        ui.end_row();
                                    });
                                });
                                ...
                                ui.collapsing("Spinlocks", |ui| {
                                    ...
                                    if let Some(ref mut spinlocks) = hyperv.spinlocks {
                                        grid(ui, "spinlocks_grid", 2, |ui| {
                                            ...
                                            ui.add(
                                                egui::Slider::new(retries, 4095..=4294967295)
                                                    .text("次"),
                                            );
                                            ui.end_row();
                                        });
                                ...
                                ui.add_space(5.0);
                                ui.collapsing("Stimer", |ui| {
                                    if hyperv.stimer.is_none() {
                                        hyperv.stimer = Some(StimerConfig {
                                            state: "off".to_string(),
                                            direct: None,
                                        });
                                    }
                                ...
                                ui.add_space(5.0);
                                ui.collapsing("Vendor ID", |ui| {
                                    if hyperv.vendor_id.is_none() {
                                        hyperv.vendor_id = Some(VendorIdConfig {
                                            state: "off".to_string(),
                                            value: None,
                                        });
                                    }
            ...
            ui.allocate_ui_with_layout(
                ...
                Layout::top_down(Align::LEFT),
                |ui| {
                    card_group(ui, "KVM 特性", Some("🐧"), colors, |ui| {
                        if let Some(ref mut features) = config.hypervisor_features {
                            ...
                            if let Some(ref mut kvm) = features.kvm {
                                ...
                                ui.add_space(5.0);
                                ui.collapsing("Dirty Ring", |ui| {
                                    if kvm.dirty_ring.is_none() {
                                        kvm.dirty_ring = Some(DirtyRingConfig {
                                            state: "off".to_string(),
                                            size: None,
                                        });
                                    }
            ...
            ui.allocate_ui_with_layout(
                ...
                Layout::top_down(Align::LEFT),
                |ui| {
                    card_group(ui, "Xen 特性", Some("☁"), colors, |ui| {
                        if let Some(ref mut features) = config.hypervisor_features {
                            ...
                            if let Some(ref mut xen) = features.xen {
                                ...
                                ui.add_space(5.0);
                                ui.collapsing("Passthrough", |ui| {
                                    if xen.passthrough.is_none() {
                                        xen.passthrough = Some(PassthroughConfig {
                                            state: "off".to_string(),
                                            mode: None,
                                        });
                                    }
            ...
            ui.allocate_ui_with_layout(
                ...
                Layout::top_down(Align::LEFT),
                |ui| {
                    card_group(ui, "其他配置", Some("⚙"), colors, |ui| {
                        if let Some(ref mut features) = config.hypervisor_features {
                            // GIC 配置
                            ui.collapsing("GIC (通用中断控制器)", |ui| {
                                ...
                                if checkbox(ui, &mut has_gic, "启用 GIC") {
                                    if has_gic {
                                        features.gic =
                                            Some(GicConfig { version: Some("2".to_string()) });
                                    } else {
                                        features.gic = None;
                                    }
                                }
                                if let Some(ref mut gic) = features.gic {
                                    ui.add_space(5.0);
                                    let version =
                                        gic.version.get_or_insert_with(|| "2".to_string());
                                ...
                                if let Some(ref mut smm) = features.smm {
                                    ...
                                    grid(ui, "smm_grid", 2, |ui| {
                                        ...
                                        if let Some(ref mut tseg) = smm.tseg {
                                            let unit =
                                                tseg.unit.get_or_insert_with(|| "MiB".to_string());
                                            ...
                                            ui.label("大小:");
                                            ui.add(
                                                egui::Slider::new(&mut tseg.value, 0..=1024)
                                                    .text(""),
                                            );
                                            ui.end_row();
                                        }
                            ...
                            ui.collapsing("IOAPIC", |ui| {
                                ...
                                if let Some(ref mut ioapic) = features.ioapic {
                                    ui.add_space(5.0);
                                    let driver =
                                        ioapic.driver.get_or_insert_with(|| "kvm".to_string());
            ...
            ui.allocate_ui_with_layout(
                ...
                Layout::top_down(Align::LEFT),
                |ui| {
                    card_group(ui, "高级配置", Some("🔬"), colors, |ui| {
                        if let Some(ref mut features) = config.hypervisor_features {
                            // HPT 配置
                            ui.collapsing("HPT (哈希页表)", |ui| {
                                ...
                                if let Some(ref mut hpt) = features.hpt {
                                    ...
                                    grid(ui, "hpt_grid", 2, |ui| {
                                        ...
                                        let resizing = hpt
                                            .resizing
                                            .get_or_insert_with(|| "enabled".to_string());
                                        ...
                                        if let Some(ref mut maxpagesize) = hpt.maxpagesize {
                                            let unit = maxpagesize
                                                .unit
                                                .get_or_insert_with(|| "MiB".to_string());
                                            ...
                                            ui.label("大小:");
                                            ui.add(
                                                egui::Slider::new(&mut maxpagesize.value, 1..=1024)
                                                    .text(""),
                                            );
                                            ui.end_row();
                                        }
                            ...
                            ui.collapsing("MSRs", |ui| {
                                ...
                                if let Some(ref mut msrs) = features.msrs {
                                    ui.add_space(5.0);
                                    let unknown =
                                        msrs.unknown.get_or_insert_with(|| "".to_string());
                            ...
                            ui.collapsing("TCG", |ui| {
                                ...
                                if let Some(ref mut tcg) = features.tcg {
                                    if tcg.tb_cache.is_none() {
                                        tcg.tb_cache = Some(TbCacheConfig {
                                            unit: Some("MiB".to_string()),
                                            value: 128,
                                        });
                                    }
                                    if let Some(ref mut tb_cache) = tcg.tb_cache {
                                        ...
                                        grid(ui, "tb_cache_grid", 2, |ui| {
                                            ...
                                            ui.label("大小:");
                                            ui.add(
                                                egui::Slider::new(&mut tb_cache.value, 1..=1024)
                                                    .text(""),
                                            );
                                            ui.end_row();
                                        });
                            ...
                            ui.collapsing("AIA", |ui| {
                                ...
                                if checkbox(ui, &mut has_aia, "启用 AIA") {
                                    if has_aia {
                                        features.aia =
                                            Some(AiaConfig { value: "aplic-imsic".to_string() });
                                    } else {
                                        features.aia = None;
                                    }
                                }
